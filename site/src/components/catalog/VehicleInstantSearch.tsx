  ...
  return (
    <div className="relative w-full">
      ...
      <input
        ...
        readOnly
        value=""
        className="w-full cursor-pointer rounded-full bg-white/[0.06] py-3 pl-11 pr-4 text-white transition-colors placeholder:text-white/60 focus:bg-white/10 focus:outline-none focus:ring-2 focus:ring-aguiar-red/40"
      />

      {open && (
        <div className="fixed inset-0 z-[100] flex flex-col bg-graphite text-white">
          <div className="flex items-center gap-3 border-b border-white/10 px-4 py-4 sm:px-8">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
            ...
            {query && (
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/25 text-white transition-colors hover:bg-white/10"
              >
                ✕
              </button>
            )}
          ...
          {brands.length > 0 && (
            <div className="flex gap-2 overflow-x-auto border-b border-white/10 px-4 py-3 sm:px-8">
              {brands.map((brand) => {
            ...
            {results.length === 0 ? (
              ...
              <ul className="flex flex-col">
                {results.map((vehicle) => {
                  ...
                  return (
                    <li key={vehicle.id} className="border-b border-white/10 last:border-b-0">
                      {/*
                      ...
                      */}
                      ...
                      >
                        {imageUrls[vehicle.id] ? (
                          ...
                          <div className="h-16 w-20 shrink-0 rounded-md bg-white/10" />
                        )}
