...
describe('Depoimentos', () => {
  ...
  it('asks for the review in plain, warm words', async () => {
    const client = fakeClient([{ id: '1', image_url: 'https://x/1.jpg', caption: 'Legenda' }])
    render(await Depoimentos({ client }))
    expect(screen.getByText('A sua opinião vale ouro para a Aguiar')).toBeInTheDocument()
    expect(screen.getByText(/como foi seu atendimento na aguiar veículos\? conte pra gente! sua opinião nos ajuda a melhorar/i)).toBeInTheDocument()
    expect(screen.getByText(/dá mais confiança a quem está procurando o próximo carro/i)).toBeInTheDocument()
  })
  ...
  describe('the format of the invitation', () => {
    ...
    it('is a very rounded card, with the Google mark on the left, the text in the middle and the button on the right', async () => {
      ...
      expect(first).toContainElement(screen.getByText('A sua opinião vale ouro para a Aguiar'))
    ...
    it('keeps the stars, the title and the text exactly as they were', async () => {
      ...
      expect(screen.getByText('A sua opinião vale ouro para a Aguiar')).toHaveClass('text-xl', 'font-bold', 'md:text-2xl')
      expect(screen.getByText(/conte pra gente! sua opinião nos ajuda a melhorar e dá mais confiança a quem está procurando o próximo carro\./i)).toBeInTheDocument()
    })
  ...
  it('passes the light background on to the carousel and to the Google invitation', async () => {
    ...
    const title = screen.getByText('A sua opinião vale ouro para a Aguiar')
    const panel = screen.getByTestId('google-cta')
    expect(panel).toHaveClass('bg-white')
    expect(title).not.toHaveClass('text-white')
    expect(screen.getByText(/conte pra gente! sua opinião nos ajuda a melhorar/i)).toHaveClass('text-graphite/70')
    const link = screen.getByRole('link', { name: /avaliar a aguiar no google/i })
