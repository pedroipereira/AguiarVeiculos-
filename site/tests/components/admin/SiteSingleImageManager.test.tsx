...
describe('SiteSingleImageManager', () => {
  ...
  it('shows "Adicionar foto" and no preview when there is no image yet', () => {
    render(<SiteSingleImageManager slot="hero" title="Hero" description="" />)
    expect(screen.queryByRole('img')).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: /adicionar foto/i })).toBeDisabled()
  })

  it('shows the current photo and "Trocar foto" when one already exists', () => {
  ...
  it('uploads and replaces the photo on save', async () => {
    ...
    const button = screen.getByRole('button', { name: /adicionar foto/i })
